...
function BlogDetail() {
        ...
        const fetchBlog = async () => {
            try {
                const res = await fetch(
                    `${import.meta.env.VITE_API_BASE_URL}/blog/${blogId}`
                );
                ...
                const fetchAll = related.map((id) =>
                    fetch(`${import.meta.env.VITE_API_BASE_URL}/product/${id}`).then((r) =>
                        r.json()
                    )
                );
    ...
    // Helper để xử lý size và màu sắc unique
    const getUniqueSizes = (variants) => {
        return [
            ...new Set(
                variants?.map((v) =>
                    v.productSize.startsWith("SIZE_")
                        ? v.productSize.replace("SIZE_", "")
                        : v.productSize
                )
            ),
        ];
    ...
    const getUniqueColors = (variants) => {
        return [
            ...new Map(
                variants?.map((v) => [v.color?.colorHex, v.color?.colorHex])
            ).values(),
        ];
    };
